...
var translations = map[locale]map[string]string{
	localeEnglish: {
		...
		"debug.description":          "Inspect which enabled catalog services and user selections cover an IP address or subnet. User coverage is calculated before route allow/deny filters.",
	...
	localeRussian: {
		...
		"debug.description":          "Показывает, какие сервисы включённых фидов и выборы пользователей покрывают IP-адрес или подсеть. Покрытие пользователей считается до применения allow/deny-фильтров маршрутов.",
