...
var translations = map[locale]map[string]string{
	localeEnglish: {
		...
		"settings.default_web_auth_hint": "Default web authentication mode for new users: network (IP only), login (credentials), both (IP + credentials). Environment variable: WDBGP_DEFAULT_WEB_AUTH.",
	...
	localeRussian: {
		...
		"settings.default_web_auth_hint": "Режим веб-аутентификации по умолчанию для новых пользователей: network (только IP), login (учетные данные), both (IP + учетные данные). Переменная окружения: WDBGP_DEFAULT_WEB_AUTH.",
