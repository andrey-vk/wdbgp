...
`); err != nil {
	...
	hasOld, err := v032TableHasColumn(ctx, db, "catalog_entries", "category")
	if err != nil {
		return err
	}
	exists, err := v032TableExists(ctx, db, "catalog_entries")
	if err != nil {
		return err
	}
	if hasOld || !exists {
...
func v032SeedDictionaries(ctx context.Context, db *sql.DB) error {
	...
	for _, src := range sources {
		oldShape, err := v032TableHasColumn(ctx, db, src.table, src.categoryCol)
		if err != nil {
			return err
		}
		if !oldShape {
...
func v032Rebuild(ctx context.Context, db *sql.DB, table, oldColumn, newShape, copySQL string) error {
	exists, err := v032TableExists(ctx, db, table)
	if err != nil {
		return err
	}
	if !exists {
		newExists, err := v032TableExists(ctx, db, table+"_new")
		if err != nil {
			return err
		}
		var sql string
	...
	hasOld, err := v032TableHasColumn(ctx, db, table, oldColumn)
	if err != nil {
		return err
	}
	if !hasOld {
...
func v032TableExists(ctx context.Context, db *sql.DB, table string) (bool, error) {
	var n int
...
func v032TableHasColumn(ctx context.Context, db *sql.DB, table, column string) (bool, error) {
	var n int
	err := db.QueryRowContext(ctx,
