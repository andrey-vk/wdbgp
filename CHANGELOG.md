...
### Added
- **New builtin "ASN" feed adapter.** Fetches announced prefixes for a list of AS numbers from RIPEstat (`stat.ripe.net/data/announced-prefixes`) and assigns them all to a single category/service. Feed `Data` must be JSON `{"asns": [...], "category": "...", "service": "..."}` — all three are mandatory and the sync fails validation without any of them. Prefixes are deduplicated across ASNs. The query is pinned to prefixes visible right now (`starttime`/`endtime` set to the current time, instead of RIPEstat's default 2-week lookback) and includes low-visibility announcements (`min_peers_seeing=1`, overriding RIPEstat's default of 10); a RIPEstat error/maintenance response fails the sync instead of silently withdrawing every prefix for the feed. Seeding this (or any future) builtin under a name that collides with an existing custom adapter renames the custom adapter out of the way rather than silently skipping the builtin or misidentifying the custom adapter as one — feeds reference adapters by id, so the rename can't break an existing feed.
- **Catalog modes can now exclude feeds.** Each feed linked to a mode carries a role: *include* (the previous behavior of every link) contributes its entries to the mode's catalog, *exclude* subtracts its prefixes from the mode's built list. The same feed can be an include in one mode and an exclude in another. Subtraction is per-prefix with no aggregation — prefixes untouched by the exclude set pass through byte-identical, overlapped ones are hole-punched into fragments that keep their original category/service (and hence their communities); exclusion is mode-global, subtracting from every service, with the exclude feed's own labels ignored and never shown in the catalog. The merge result is materialized per mode in a new `catalog_mode_entries` table (migration 036), rebuilt whole whenever a member feed syncs, a membership or role changes, or a feed is enabled/disabled/deleted — so the heavy work runs once per sync per affected mode, never on the per-user reconcile path, and the BGP announcement, catalog, and prefix-count queries all got simpler and faster by reading the materialized table instead of re-joining feeds on every call. The Modes admin page gets a per-feed include/exclude toggle.
...
### Removed
- **The plaintext password-in-OPEN fallback for loopback peers.** It embedded the BGP password as a raw optional parameter (the deprecated RFC 1771 type 1) in the OPEN message and compared it on the other side — a wdbgp-only dialect no other BGP implementation speaks, so it could only ever authenticate wdbgp against wdbgp over loopback, which in practice meant the test suite. Loopback sessions are now plainly unauthenticated (many kernels, WSL2 included, can't enforce TCP MD5 on loopback; loopback traffic is same-host anyway) — a configured password is ignored there instead of feeding a make-believe handshake. Real peers are unaffected: TCP MD5 (RFC 2385) and the NFQUEUE dynamic-peer matcher remain the authentication mechanisms, and the OPEN message no longer ever carries password material.

### Fixed
- **A slow full-table send no longer masquerades as a hold-timer expiry.** The session loop armed the read deadline before servicing pending route updates, so a resync that outlasted the hold time (large table, slow peer) made the very next read fail instantly — tearing down a live session whose keepalives were sitting unread in the buffer — and logging it as `hold timer expired`. The deadline is now re-armed after a resync completes. Pre-existing bug; route-refresh support (above) made it much easier to trigger.
